  ...
  def show_friendship(user)
    ...
    if send_friendship
      link_to('Cancel Friendship Request',
              friendship_path(id: send_friendship[:id], sender_id: current_user[:id], receiver_id: user[:id]), method: :DELETE, class: 'btn-link')
    elsif pending_friendship
      link_to('Accept Friendship Request',
              friendship_path(id: pending_friendship[:id], sender_id: user[:id], receiver_id: current_user[:id]), method: :PUT, class: 'btn-link') +
      ...
      link_to('Remove Friendship',
              friendship_path(id: accepted_friendship[:id], sender_id: user[:id], receiver_id: current_user[:id]), method: :DELETE, class: 'btn-link')
    else
